...
df = pd.read_csv("data_iops.csv",header=0)
print(df)
df = df.T
#df = df.dropna()
df[1:] = df[1:] / 1000000
...
ax.set_ylabel("Throughput (Mops)", fontsize=38)

# Legend style
a = ax.legend().get_texts()
label = [x.get_text() for x in a]
l = ax.legend(labels= label[:4], loc='center left', ncols= 2, bbox_to_anchor=(0.09, 0.90), fontsize=36, reverse=False, labelspacing=0.3, columnspacing=0.5, frameon=False, handlelength=0.8, handleheight=1, markerscale=0.8, handletextpad=0.3)
