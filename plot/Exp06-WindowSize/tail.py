...
ax.set_ylim(125, 150)
ax.set_yticks(np.arange(125, 151, 5))

minor_yticks = np.arange(130, 150, 10)
...
ax.set_xlabel("Window size", fontsize=42)
ax.set_ylabel(r"Tail latency ($\mu$s)", fontsize=42)

# Legend style
a = ax.legend().get_texts()
label = [x.get_text() for x in a]
l = ax.legend(labels= label[:4], loc='center left', ncols= 2, bbox_to_anchor=(0.1, 0.90), fontsize=36, reverse=False, labelspacing=0.3, columnspacing=0.5, frameon=False, handlelength=0.8, handleheight=1, markerscale=0.8, handletextpad=0.3)
